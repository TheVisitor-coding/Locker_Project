...
function submit(){
    ...
    if(( match !== null && match.length === 1) && code.length === 4){
        ...
        fetch('http://10.54.133.151/locker/MDSlocker/includes/request.php?url='+'http://10.54.128.96:8888/projetworkshop/index.php'+'&params=code='+code)
            .then(response => response.json())
            .then(response => {
                const code = response.code;
                const pin = response.pin;
                if (response.errors === undefined) {
                    fetch('http://10.54.133.151/locker/MDSlocker/includes/send.php?pin='+pin+"&code="+code)
                        .then(response => {
...
function displayOpenLocker(){
    fetch('http://10.54.133.151/locker/MDSlocker/includes/request.php?params=isOpen=1&url=http://10.54.128.96:8888/projetworkshop/locker-list.php')
        .then(response => response.json())
        .then(response => {
            ...
            if(Array.isArray(response)) {
                ...
                for(const locker of response) {
                    ...
                    lockerElement.children[1].setAttribute('code',locker.code);
...
function closeLockerEvent(){
    ...
    closeButton.forEach(el => {
        el.onclick = (e) => {
            ...
            fetch('http://10.54.133.151/locker/MDSlocker/includes/close.php?pin='+el.getAttribute('pin')+"&code="+el.getAttribute("code"))
